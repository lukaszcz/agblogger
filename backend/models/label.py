...
class LabelParentCache(Base):
    ...
    label_id: Mapped[str] = mapped_column(
        String, ForeignKey("labels_cache.id"), primary_key=True
    )
    parent_id: Mapped[str] = mapped_column(
        String, ForeignKey("labels_cache.id"), primary_key=True
    )

    label: Mapped["LabelCache"] = relationship(
        foreign_keys=[label_id], back_populates="parent_edges"
    )
...
class PostLabelCache(Base):
    ...
    source: Mapped[str] = mapped_column(
        String, nullable=False, default="frontmatter"
    )

    post: Mapped["PostCache"] = relationship(  # type: ignore[name-defined]  # noqa: F821
