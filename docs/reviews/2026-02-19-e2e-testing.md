...
### Suggestions for Improvement (all resolved)

1. ~~**No post delete in UI**~~ — Added Delete button to post view (auth only) with confirmation dialog. Navigates to timeline after deletion.
2. ~~**Editor 404 state**~~ — Editor now shows a prominent 404/Error page without the editor form when loading a nonexistent post.
3. ~~**Post list initial load flash**~~ — Replaced spinner with 3 skeleton cards matching PostCard shape during initial load.
4. ~~**Search page has no refinement input**~~ — Added search input field pre-filled with current query, Search button, and result count summary.
5. ~~**Logout button ambiguity**~~ — Added `title="Log out"` tooltip to the logout button.
...
just check: all passing
  ...
  Frontend: tsc clean, eslint clean, 67 tests passed
```
